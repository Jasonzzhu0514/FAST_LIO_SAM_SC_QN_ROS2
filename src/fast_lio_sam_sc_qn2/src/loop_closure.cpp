...
{
    std::lock_guard<std::mutex> lock(sc_mutex_);
    const auto sc_detected = sc_manager_.detectLoopClosureIDGivenScan(query_keyframe.pcd_);
    const int candidate_keyframe_idx = sc_detected.first;
        ...
        const double distance =
            (keyframes[candidate_keyframe_idx].pose_corrected_eig_.block<3, 1>(0, 3) -
             query_keyframe.pose_corrected_eig_.block<3, 1>(0, 3))
                .norm();
        if (distance < config_.scancontext_max_correspondence_distance_)
        {
                                       ...
                                       const bool enable_submap_matching)
{
    ...
    const int num_approx = static_cast<int>(keyframes[src_idx].pcd_.size()) * 2 * submap_range;
                                                      ...
                                                      const pcl::PointCloud<PointType> &dst)
{
    ...
    const auto quatro_tf = reg_output.pose_between_eig_;
    reg_output = fine_output;
    reg_output.pose_between_eig_ = fine_output.pose_between_eig_ * quatro_tf;
    return reg_output;
}
                                                   ...
                                                   const int closest_keyframe_idx)
{
    ...
    *dst_cloud_ = dst_cloud;
    ...
    {
        std::cout << "Execute coarse-to-fine alignment: " << src_cloud.size()
                  << " vs " << dst_cloud.size() << "\n";
        return coarseToFineAlignment(src_cloud, dst_cloud);
    }

    std::cout << "Execute GICP: " << src_cloud.size() << " vs " << dst_cloud.size() << "\n";
